...
def main():
    ...
    while cash > 0:
        ...
        if Product_list[prod_id].getProductquantity() >= count:
            if cash >= Product_list[prod_id].getProductprice():
                Product_list[prod_id].getProductquantity() * count
                price = Product_list[prod_id].getProductprice() * count
                cash -= price
                print("You purchased", count, Product_list[prod_id].getProductname() + " for $" + str(price) + ".")
